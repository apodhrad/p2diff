...
public class DiffManager {
	...
	private void generateDiff() throws IOException {
		...
		ResourceUtils.listf(zip1.getPath(), filesInZip1);
		ResourceUtils.listf(zip2.getPath(), filesInZip2);
				...
				if (file1.getName().matches(filter) && file2.getName().matches(filter)) {
					...
					if (!(file1.getName().equals(zip1.getName()) || file2.getName().equals(zip2.getName()))) {
						if (!(file1.isDirectory() || file2.isDirectory())) {
							...
							if ((FilenameUtils.getExtension(file1.getPath()).equals("jar"))
									&& (FilenameUtils.getExtension(file2.getPath()).equals("jar"))) {
